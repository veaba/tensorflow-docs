...
- `input`_min: If range_given=True, the minimum `input` value that needs to be represented in the quantized representation.
- `input`_max: If range_given=True, the maximum `input` value that needs to be represented in the quantized representation.
- `signed_input`: True if the quantization is signed or unsigned.
- `num_bits`: The bitwidth of the quantization.
- `range_given`: If true use `input`_min and `input`_max for the range of the `input`, otherwise determine min and max from the `input` `Tensor`.
