...
[Example](https://www.tensorflow.org/code/tensorflow/core/example/example.proto)Parses a number of serialized  protos given in serialized. We refer to serialized as a batch with batch_size many entries of individual  protos.
...
[tf.float32](https://www.tensorflow.org/api_docs/python/tf#float32)Each FixedLenSequenceFeature df maps to a Tensor of the specified type (or  if not specified) and shape (serialized.size(), None) + df.shape. All examples in serialized will be padded with default_value along the second dimension.

#### Examples:
[tf.float32](https://www.tensorflow.org/api_docs/python/tf#float32)For example, if one expects a  VarLenFeature ft and three serialized Examples are provided:
