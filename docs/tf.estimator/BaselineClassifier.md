...
- **`n_classes`** : number of label classes. Default is binary classification.It must be greater than 1. 注意：Class labels are integers representingthe class index (i.e. values from 0 to n_classes-1). For arbitrarylabel values (e.g. string labels), convert to class indices first.
