...
具有自定义头的Tensorflow 线性和dnn连接模型的估计量。
...
初始化dnnlinearcombinedestimator实例。

#### 参数：
- **`head`** : A  `_Head`  instance constructed with a method such as `tf.contrib.estimator.multi_label_head` .
- **`model_dir`** : Directory to save model parameters, graph and etc. This canalso be used to load checkpoints from the directory into an estimatorto continue training a previously saved model.
