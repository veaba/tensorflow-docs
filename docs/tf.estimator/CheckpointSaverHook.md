...
- `checkpoint_basename`: `str`, base name for the checkpo`int` files.
- `scaffold`: `Scaffold`, use to get `saver` object.
- `listeners`: List of `CheckpointSaverListener` subclass instances. Used for callbacks that run immediately before or after this hook saves the checkpo`int`.
#### Raises:
