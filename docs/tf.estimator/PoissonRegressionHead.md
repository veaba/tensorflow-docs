...
**Aliases** : [ `tf.compat.v1.estimator.PoissonRegressionHead` ](/api_docs/python/tf/estimator/PoissonRegressionHead), [ `tf.compat.v2.estimator.PoissonRegressionHead` ](/api_docs/python/tf/estimator/PoissonRegressionHead)
...
- **`compute_full_loss`** : Whether to include the constant  `log(z!)`  term incomputing the poisson loss. See [ `tf.nn.log_poisson_loss` ](https://tensorflow.google.cn/api_docs/python/tf/nn/log_poisson_loss) for the fulldocumentation.
