...
version](/versions/r1.15/api_docs/python/tf/estimator/Estimator) |  [
...
The `config` argument can be passed
[`tf.estimator.RunConfig`](https://tensorflow.google.cn/api_docs/python/tf/estimator/RunConfig)
object containing information about the execution environment. It is passed on
to the `model_fn`, if the `model_fn` has a parameter named "config" (and input
