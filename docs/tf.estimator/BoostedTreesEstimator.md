...
Tensorflow 增强树模型的估计量。

**别名** : [ `tf.compat.v1.estimator.BoostedTreesEstimator` ](/api_docs/python/tf/estimator/BoostedTreesEstimator), [ `tf.compat.v2.estimator.BoostedTreesEstimator` ](/api_docs/python/tf/estimator/BoostedTreesEstimator)
...
Use this interface if you need to provide a custom loss/head.For example, the following will be equivalent to usingBoostedTreesRegressor

# Create a head with L2 loss
from tensorflow_estimator.python.estimator.canned importhead_lib
...
- **`n_batches_per_layer`** : the number of batches to collect statistics perlayer.
- **`head`** : the  `Head`  instance defined for Estimator.
- **`model_dir`** : Directory to save model parameters, graph and etc. This canalso be used to load checkpoints from the directory into an estimator tocontinue training a previously saved model.
- **`weight_column`** : A string or a  `_NumericColumn`  created by[ `tf.feature_column.numeric_column` ](https://tensorflow.google.cn/api_docs/python/tf/feature_column/numeric_column) defining feature column representingweights. It is used to downweight or boost examples during training. Itwill be multiplied by the loss of the example. If it is a string, it isused as a key to fetch weight tensor from the  `features` . If it is a `_NumericColumn` , raw tensor is fetched by key  `weight_column.key` , thenweight_column.normalizer_fn is applied on it to get weight tensor.
...
- **`min_node_weight`** : minimum hessian a node must have for a split to beconsidered. The value will be compared with sum(leaf_hessian)/(batch_size * n_batches_per_layer).
