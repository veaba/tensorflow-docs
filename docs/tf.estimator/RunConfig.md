...
`task` has two attributes: `type` and `index`, where `type` can be any of the `task` `type`s in `cluster`. When `TF_CONFIG` contains said information, the following properties are set on this class:
