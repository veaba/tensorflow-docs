...
#### 参数：
- **`head`** : A  `Head`  instance. This specifies the model's output and lossfunction to be optimized.
...
- **`return_sequences`** : A boolean indicating whether to return the last outputin the output sequence, or the full sequence.
...
Note that a RNN cell is has:  - a  `call`  method.  - a  `state_size`  attribute.  - a  `output_size`  attribute.  - a  `get_initial_state`  method.See the documentation on [ `tf.keras.layers.RNN` ](https://tensorflow.google.cn/api_docs/python/tf/keras/layers/RNN) for more details.
