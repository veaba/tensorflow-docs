...
```
This operation is sometimes called "de`convolution`" after De`convolution`al Networks, but is actually the transpose (gradient) of `convolution` rather than an actual de`convolution`.
#### Args:
