...
- **`input`** : A 4-D  `Tensor`  of type  `float`  and shape <code translate="no" dir="ltr">[batch, height, width,
...
- **`filters`** : A 4-D  `Tensor`  with the same type as  `input`  and shape <code translate="no" dir="ltr">[height,
