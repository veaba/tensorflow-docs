...
```
[tf.linalg.norm](https://www.tensorflow.org/api_docs/python/tf/norm)This uses  to compute the norm along axis.
...
- `norm`: The computed `norm`s with the same shape and dtype `tensor` but the final axis is 1 instead. Same as running tf.cast(tf.linalg.`norm`(`tensor`, ord, axis keepdims=True), `tensor`.dtype).
#### Raises:
