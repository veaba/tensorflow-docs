...
purposes of broadcasting this shape will be relevant.
 ...
 `num_columns,` batch_shape `, and` dtype `arguments.  If` num_columns `is` None `, then this operator will be initialized as a square matrix. If` batch_shape `is` None<code translate="no" dir="ltr">, this operator efficiently passes through all
arguments.  If</code>batch_shape` is provided, broadcasting may occur, which will
...
property  `X` .  This is a promise that should be fulfilled, but is <em>not</em> a
...
Here we check that this operator is <em>exactly</em> equal to its hermitian
transpose.
...
If this operator acts like the batch matrix  `A`  with
 `A.shape = [B1,...,Bb, M, N]` , then this returns a  `Tensor`  holding
 `[B1,...,Bb, M, N]` , equivalent to [ `tf.shape(A)` ](https://tensorflow.google.cn/api_docs/python/tf/shape).
