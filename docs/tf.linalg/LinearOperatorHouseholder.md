...
#### Args:
- `reflection_axis`:` `Shape` `[`B``1``,``.``.``.``,``B``b``,`` ``N`]` ``Tensor`` `with` ``b`` `>=` `0` ``N`` `>=` `0`.`` `The` `vector` `defining` `the` `hyperplane` `to` `reflect` `a`b`out`.`` `Allowed` `dtypes:` ``float16``,`` ``float32``,`` ``float64``,`` ``complex64``,`` ``complex128``.`
...
- `is_square`:` `E`x`pect` `that` `this` `operator` `acts` `like` `square` `[`b`atch]` `matrices`.`` `This` `is` `autoset` `to` `true`.`
