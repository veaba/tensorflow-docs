...
[tf.device](https://tensorflow.google.cn/api_docs/python/tf/device)1) It is generally not recommended to open a device scope under the strategy's scope. A device scope (i.e. calling ) will be merged with or override the device for operations but will not change the device for variables.

[tf.compat.v1.colocate_with](https://tensorflow.google.cn/api_docs/python/tf/compat/v1/colocate_with)2) It is also not recommended to open a colocation scope (i.e. calling ) under the strategy's scope. For colocating variables, use strategy.extended.colocate_vars_with instead. Colocation of ops will possibly create device assignment conflicts.
