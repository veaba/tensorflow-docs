...
[the guide](https://www.tensorflow.org/alpha/guide/distribute_strategy)See  for overview and examples.
...
```
[tf.distribute.Strategy](https://www.tensorflow.org/api_docs/python/tf/distribute/Strategy)This takes an ordinary dataset and replica_fn and runs it distributed using a particular  named my_strategy above. Any variables created in replica_fn are created using my_strategy's policy, and library functions called by replica_fn can use the get_replica_context() API to implement distributed-specific behavior.

[tf.keras.metrics](https://www.tensorflow.org/api_docs/python/tf/keras/metrics)You can use the reduce API to aggregate results across replicas and use this as a return value from one iteration over the distributed dataset. Or you can use  (such as loss, accuracy, etc.) to accumulate metrics across steps in a given epoch.

[custom training loop tutorial](https://www.tensorflow.org/alpha/tutorials/distribute/training_loops)See the  for a more detailed example.
