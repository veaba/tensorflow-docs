...
original image. The output is returned as 3 tensors:  `begin` ,  `size`  and
 `bboxes` . The first 2 tensors can be fed directly into [ `tf.slice` ](https://tensorflow.google.cn/api_docs/python/tf/slice) to crop the
image. The latter may be supplied to [ `tf.image.draw_bounding_boxes` ](https://tensorflow.google.cn/api_docs/python/tf/image/draw_bounding_boxes) to
...
- **`aspect_ratio_range`** : An optional list of  `floats` . Defaults to <code translate="no" dir="ltr">[0.75,
1.33]</code>. The cropped area of the image must have an aspect <code translate="no" dir="ltr">ratio = width /
height</code> within this range.
...
Provide as input to [ `tf.image.draw_bounding_boxes` ](https://tensorflow.google.cn/api_docs/python/tf/image/draw_bounding_boxes).
