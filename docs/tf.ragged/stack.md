...
**Aliases** : [ `tf.compat.v1.ragged.stack` ](/api_docs/python/tf/ragged/stack), [ `tf.compat.v2.ragged.stack` ](/api_docs/python/tf/ragged/stack)
...
* **`axis`**: A python integer, indicating the dimension along which to stack.
  (Note: Unlike [tf.stack](../../tf/stack), the `axis` parameter must be statically known.)
