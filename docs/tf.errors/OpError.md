...
[tf.errors](https://www.tensorflow.org/api_docs/python/tf/errors)Whenever possible, the session will raise a more specific subclass of OpError from the  module.
...
#### Args:
- `node_def`: The `node_def`_pb2.NodeDef proto representing the op that failed, if known; otherwise None.
- `op`: The `op`s.Operation that failed, if known; otherwise None.
- `message`: The `message` string describing the failure.
- `error_code`: The `error_code`s_pb2.Code describing the error.
## Properties
### error_code
