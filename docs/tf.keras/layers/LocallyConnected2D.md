...
Also, only  `padding="valid"`  is supported by  `implementation=1` .
    ...
    - np.prod(strides))<code translate="no" dir="ltr">, where inputs to and outputs of the layer are
assumed to have shapes</code>input_size + (input_filters,) `,` output_size + (filters,)` respectively.
