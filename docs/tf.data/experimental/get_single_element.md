...
```
[tf.data.Dataset](https://tensorflow.google.cn/api_docs/python/tf/data/Dataset)This function enables you to use a  in a stateless "tensor-in tensor-out" expression, without creating a tf.compat.v1.data.Iterator. This can be useful when your preprocessing transformations are expressed as a Dataset, and you want to use the transformation at serving time. For example:
...
#### Returns:
[tf.Tensor](https://tensorflow.google.cn/api_docs/python/tf/Tensor)A nested structure of  objects, corresponding to the single element of dataset.
