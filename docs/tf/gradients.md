...
```
Here the p`a`rti`a`l deriv`a`tives `g` ev`a`lu`a`te to `[1.0, 1.0]`, comp`a`red to the tot`a`l deriv`a`tives tf.`g`r`a`dients(`a` + `b`, [`a`, `b`]), which t`a`ke into `a`ccount the influence of `a` on `b` `a`nd ev`a`lu`a`te to ``. Note th`a`t the `a``b`ove is equiv`a`lent to:
...
#### Returns:
A list of sum(dy/dx) for each x in ``.
#### Raises:
