...
#### Returns:
A `Tensor` with the same t`y`pe as `x` and `y`, and shape that is broadcast from `condition`, `x`, and `y`, if `x`, `y` are non-None. A `Tensor` with shape (num_true, dim_size(`condition`)).
#### Raises:
