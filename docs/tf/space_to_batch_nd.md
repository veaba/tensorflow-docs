...
**Aliases** : [ `tf.compat.v1.manip.space_to_batch_nd` ](/api_docs/python/tf/space_to_batch_nd), [ `tf.compat.v1.space_to_batch_nd` ](/api_docs/python/tf/space_to_batch_nd), [ `tf.compat.v2.space_to_batch_nd` ](/api_docs/python/tf/space_to_batch_nd)
...
(1) For the following input of shape  `[1, 2, 2, 1]` ,  `block_shape = [2, 2]` , and   `paddings = [[0, 0], [0, 0]]` :
    ...
    1. Zero-pad the start and end of dimensions  `[1, ..., M]`  of theinput according to  `paddings`  to produce  `padded`  of shape  `padded_shape` .
...
block_shape +[batch] +[padded_shape[1] / block_shape[0],...,padded_shape[M] / block_shape[M-1]] +remaining_shape
    ...
    4. Reshape  `permuted_reshaped_padded`  to flatten  `block_shape`  into the batchdimension, producing an output tensor of shape:
