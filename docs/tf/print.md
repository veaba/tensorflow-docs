...
In graphs manually created outside of [ `tf.function` ](https://tensorflow.google.cn/api_docs/python/tf/function), this method returns
the created TF operator that prints the data. To make sure the
operator runs, users need to pass the produced op to
[ `tf.compat.v1.Session` ](https://tensorflow.google.cn/api_docs/python/tf/compat/v1/Session)'s run method, or to use the op as a control
...
>
<devsite-code><pre class="" translate="no" dir="ltr" is-upgraded=""> `sess = tf.compat.v1.Session()<br>with sess.as_default():<br>&nbsp; &nbsp; tensor = tf.range(10)<br>&nbsp; &nbsp; print_op = tf.print("tensors:", tensor, {2: tensor * 2},<br>&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; output_stream=sys.stdout)<br>&nbsp; &nbsp; with tf.control_dependencies([print_op]):<br>&nbsp; &nbsp; &nbsp; tripled_tensor = tensor * 3<br>&nbsp; &nbsp; sess.run(tripled_tensor)<br>` </pre></devsite-code>
(This prints "tensors: [0 1 2 ... 7 8 9] {2: [0 2 4 ... 14 16 18]}" to
  sys.stdout)


[**Note:**  In Jupyter notebooks and colabs,  `tf.print` ](https://tensorflow.google.cn/api_docs/python/tf/print) prints to the notebook
  cell outputs. It will not write to the notebook kernel's console logs.</aside>
...
except inside of [ `tf.compat.v1` ](https://tensorflow.google.cn/api_docs/python/tf/compat/v1) graphs and sessions.
