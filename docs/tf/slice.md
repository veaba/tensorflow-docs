...
[tf.Tensor.getitem](https://www.tensorflow.org/api_docs/python/tf/Tensor#__getitem__)Note that  is typically a more pythonic way to perform slices, as it allows you to write foo[3:7, :-2] instead of tf.slice(foo, [3, 0], [4, foo.get_shape()[1]-2]).
