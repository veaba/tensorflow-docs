...
If `num_or_size_splits` `i`s a 1-D Tensor (or l`i`st), we call `i`t `size_splits` and `value` `i`s spl`i`t `i`nto len(`size_splits`) elements. The shape of the `i`-th element has the same s`i`ze as the `value` except along d`i`mens`i`on `axis` where the s`i`ze `i`s `size_splits`[`i`].
#### For example:
