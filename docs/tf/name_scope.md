...
[name_scope](https://tensorflow.google.cn/api_docs/python/tf/compat/v1/keras/backend/name_scope)Inherits From: 
...
```
When exe`c`uted, the Tensors `a`, `b`, `c`, will h`a`ve n`a`mes `MyOp/a`, `MyOp/b`, `a`nd `MyOp/c`.
