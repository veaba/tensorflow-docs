...
```
[tf.py_function](https://tensorflow.google.cn/api_docs/python/tf/py_function)You can also use  to debug your models at runtime using Python tools, i.e., you can isolate portions of your code that you want to debug, wrap them in Python functions and insert pdb tracepoints or print statements as desired, and wrap those functions in .

[Eager guide](https://tensorflow.org/guide/eager)For more information on eager execution, see the .

[tf.py_function](https://tensorflow.google.cn/api_docs/python/tf/py_function) is similar in spirit to tf.compat.v1.py_func, but unlike the latter, the former lets you use TensorFlow operations in the wrapped Python function. In particular, while tf.compat.v1.py_func only runs on CPUs and wraps functions that take NumPy arrays as inputs and return NumPy arrays as outputs,  can be placed on GPUs and wraps functions that take Tensors as inputs, execute TensorFlow operations in their bodies, and return Tensors as outputs.

[tf.compat.v1.py_func](https://tensorflow.google.cn/api_docs/python/tf/compat/v1/py_func)Like , tf.py_function has the following limitations with respect to serialization and distribution:
