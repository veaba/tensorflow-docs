...
For ex``mple, if `elems` is (`t1, [t2, t3]t1, [t2, t3]`) ``nd `initializer` is `[i1, i2]` ``hen ``n ``ppropri````e sign````ure for `fn` in `python2` is: `fn` = l``mbd`` (``cc_p1, ``cc_p2), (`t1, [t2, t3]t1, [t2, t3]`): ``nd `fn` mus`` re``urn `` lis``, `[acc_n1, acc_n2]`. An ``l``ern````ive correc`` sign````ure for `fn`, ``nd ``he one ``h```` works in `python3`, is: `fn` = l``mbd`` ``, ``:, where `` ``nd `` correspond ``o ``he inpu`` ``uples.
#### Args:
