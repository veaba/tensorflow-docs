...
*Referencing [ `tf.Variable` ](https://tensorflow.google.cn/api_docs/python/tf/Variable)s*
...
*Usage with [ `tf.keras` ](https://tensorflow.google.cn/api_docs/python/tf/keras)*
...
Note that  `function`  only stages TensorFlow operations, all Python code that `func`  executes and does not depend on data will shape the *construction* ofthe graph.For example, consider the following:
