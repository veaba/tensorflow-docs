...
Tracing and staging
When `autograph` is `True`, all Python control flow that depends on `Tensor` values is staged into a `Tensor`Flow graph. When `autograph` is `False`, the function is traced and control flow is not allowed to depend on data.
...
Python Side-Effects
A corollary of the previous discussion on tracing is the following: If a Python `func`tion `func` has Python side-effects, then executing `func` multiple times may not be semantically equivalent to executing F = tf.`func`tion(`func`) multiple times; this difference is due to the fact that `func`tion only captures the subgraph of TensorFlow operations that is constructed when `func` is invoked to trace a graph.
