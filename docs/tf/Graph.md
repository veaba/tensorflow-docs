...
[tf.Operation](https://www.tensorflow.org/api_docs/python/tf/Operation)A Graph contains a set of  objects, which represent units of computation; and tf.Tensor objects, which represent the units of data that flow between operations.

[tf.compat.v1.get_default_graph](https://www.tensorflow.org/api_docs/python/tf/compat/v1/get_default_graph)A default Graph is always registered, and accessible by calling . To add an operation to the default graph, simply call one of the functions that defines a new Operation:
...
```
[tf.Graph.as_default](https://www.tensorflow.org/api_docs/python/tf/Graph#as_default)Another typical usage involves the  context manager, which overrides the current default graph for the lifetime of the context:
...
[GraphDef](https://www.tensorflow.org/code/tensorflow/core/framework/graph.proto)For details on the meaning of each version, see .
...
[tf.Graph.graph_def_versions](https://www.tensorflow.org/api_docs/python/tf/Graph#graph_def_versions)Note that this is unrelated to the .
...
[tf.import_graph_def](https://www.tensorflow.org/api_docs/python/tf/graph_util/import_graph_def)The serialized GraphDef can be imported into another Graph (using ) or used with the C++ Session API.
...
#### Returns:
[GraphDef](https://www.tensorflow.org/code/tensorflow/core/framework/graph.proto)A  protocol buffer.
...
[tf.constant()](https://www.tensorflow.org/api_docs/python/tf/constant)This is a low-level interface for creating an Operation. Most programs will not call this method directly, and instead use the Python op constructors, such as , which add ops to the default graph.
...
[DeviceNameUtils](https://www.tensorflow.org/code/tensorflow/core/util/device_name_utils.h)For information about the valid syntax of device name strings, see the documentation in .
...
with g.device(matmul_on_gpu):
  ...
  # on CPU 0.
```
[tf.Variable](https://www.tensorflow.org/api_docs/python/tf/Variable)N.B. The device scope may be overridden by op wrappers or other library code. For example, a variable assignment op v.assign() must be colocated with the  v, and incompatible device scopes will be ignored.
...
[tf.compat.v1.train.QueueRunner](https://www.tensorflow.org/api_docs/python/tf/compat/v1/train/QueueRunner)After calling g.finalize(), no new operations can be added to g. This method is used to ensure that no operations are added to a graph when it is shared between multiple threads, for example when using a .
