...
```
[tf.gather](https://tensorflow.google.cn/api_docs/python/tf/gather)Whereas in  indices defines slices into the first dimension of params, in _nd, indices defines slices into the first N dimensions of params, where N = indices.shape[-1].
...
```
    ...
    output = [['c0'], ['b1']]
```
[tf.gather](https://tensorflow.google.cn/api_docs/python/tf/gather)See also .
