...
```
Whereas in `tf.gather` `indices` defines slices into the first dimension of `params`, in `tf.gather`_nd, `indices` defines slices into the first `N` dimensions of `params`, where `N` = `indices`.shape[-1].
