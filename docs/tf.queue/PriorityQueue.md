...
A `PriorityQueue` holds a list of up to `capacity` elements. Each element is a fixed-length tuple of tensors whose d`types` are described by `types`, and whose `shapes` are optionally described by the `shapes` argument.
...
Enqueues and Dequeues to the `PriorityQueue` must include an additional tuple entry at the beginning: the `priority`. The `priority` must be an int64 scalar (for `enqueue`) or an int64 vector (for `enqueue`_many).
#### Args:
