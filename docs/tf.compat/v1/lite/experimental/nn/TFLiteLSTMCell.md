...
- `dtype`: Default `dtype` of the layer (default of `None` means use the type of the first input). Required when `build` is `call`ed before `call`. When restoring from CudnnLSTM-trained checkpoints, use `CudnnCompatibleLSTMCell` instead.
## Properties
### graph
DEPRECATED FUNCTION
### output_size
