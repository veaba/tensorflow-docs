...
- **`output_format`** : Output file format. Currently must be <code translate="no" dir="ltr">{TFLITE,
GRAPHVIZ_DOT}</code>. (default TFLITE)
...
- **`ValueError`** :   Input shape is not specified.
...
- **`ValueError`** :   The graph is not frozen.
