...
Inputs must be a list of categorical columns created by any of the `categorical_column_`* function. They must all be of the same type and have the same arguments except `key`. E.g. they can be `categorical_column_`with_vocabulary_file with the same vocabulary_file. Some or all columns could also be weighted_`categorical_column`.
