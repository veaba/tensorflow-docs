...
- `graph`_def: DEPRECATED: Use the `graph` argument instead.
- `filename_suffix`: A string. Every event file's name is `suffix`ed with `suffix`.
...
- `graph`_def: DEPRECATED. Use the `graph` parameter instead.
#### Raises:
...
[tf.Tensor.eval](https://www.tensorflow.org/api_docs/python/tf/Tensor#eval)You can pass the result of evaluating any summary op, using tf.Session.run or , to this function. Alternatively, you can pass a tf.compat.v1.Summary protocol buffer that you populate with your own data. The latter is commonly done to report evaluation results in event files.
