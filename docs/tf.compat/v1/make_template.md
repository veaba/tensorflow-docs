...
```
This wraps `func_` in a `Template` and partially evaluates it. `Template`s are functions that create variables the first time they are called and reuse them thereafter. In order for `func_` to be compatible with a `Template` it must have the following properties:
In the follo`w`ing example, both `z` and `w` `w`ill be scaled b`y` the same `y`. It is important to note that if `w`e didn't assign `scalar_name` and used a different name for `z` and `w` that a `ValueError` `w`ould be thro`w`n because it couldn't reuse the variable.
