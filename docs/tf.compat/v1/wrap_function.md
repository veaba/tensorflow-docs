...
```
[tf.compat.v1.wrap_function](https://www.tensorflow.org/api_docs/python/tf/compat/v1/wrap_function)Both  and tf.function create a callable TensorFlow graph. But while tf.function runs all stateful operations (e.g. tf.print) and sequences operations to provide the same semantics as eager execution, wrap_function is closer to the behavior of session.run in TensorFlow 1.x. It will not run any operations unless they are required to compute the function's outputs, either through a data dependency or a control dependency. Nor will it sequence operations.

[tf.function](https://www.tensorflow.org/api_docs/python/tf/function)Unlike , wrap_function will only trace the Python function once. As with placeholders in TF 1.x, shapes and dtypes must be provided to wrap_function's signature argument.
