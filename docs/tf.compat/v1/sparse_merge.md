...
- `vocab_size`: A scalar `int64` Tensor (or Python `i`nt) conta`i`n`i`ng the new s`i`ze of the last d`i`mens`i`on, all(0 <= `sp_ids`.`values` < `vocab_size`). Or a l`i`st thereof w`i`th all(0 <= `sp_ids`[`i`].`values` < `vocab_size`[`i`]) for all `i`.
- `name`: A `name` pref`i`x for the returned tensors (opt`i`onal)
- `already_sorted`: A boolean to spec`i`fy whether the per-batch `values` `i`n `sp_values` are already sorted. If so sk`i`p sort`i`ng, False by default (opt`i`onal).
#### Returns:
