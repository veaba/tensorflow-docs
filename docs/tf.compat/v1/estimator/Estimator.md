...
#### Eager Compatibility
[tf.data](https://tensorflow.google.cn/api_docs/python/tf/data)Calling methods of Estimator will work while eager execution is enabled. However, the model_fn and input_fn is not executed eagerly, Estimator will switch to graph mode before calling all user-provided functions (incl. hooks), so their code has to be compatible with graph mode execution. Note that input_fn code using  generally works in both graph and eager modes.
...
```
[tf.estimator.WarmStartSettings](https://tensorflow.google.cn/api_docs/python/tf/estimator/WarmStartSettings)For more details on warm-start configuration, see .
