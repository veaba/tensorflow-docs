...
[tf.estimator.Estimator](https://tensorflow.google.cn/api_docs/python/tf/estimator/Estimator)TPUEstimator also supports training on CPU and GPU. You don't need to define a separate .
...
```
# Exporting
[saved_model.SERVING](https://tensorflow.google.cn/api_docs/python/tf/saved_model#SERVING)export_saved_model exports 2 metagraphs, one with , and another with  and saved_model.TPU tags. At serving time, these tags are used to select the appropriate metagraph to load.
