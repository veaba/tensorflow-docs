...
```
If `x` or `y` is comple`x`, the `J`acobian will still be real but the corresponding `J`acobian dimension(s) will be twice as large. This is required even if both input and output is comple`x` since TensorFlow graphs are not necessaril`y` holomorphic, and ma`y` have gradients not e`x`pressible as comple`x` numbers. For e`x`ample, if `x` is comple`x` with shape `[m]` and `y` is comple`x` with shape `[n]`, each `J`acobian `J` will have shape [m * 2, n * 2] with
