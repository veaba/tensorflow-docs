...
[ `tf.compat.v1.train.MonitoredTrainingSession` ](https://tensorflow.google.cn/api_docs/python/tf/compat/v1/train/MonitoredTrainingSession) instead.
...
[ `tf.errors.OutOfRangeError` ](https://tensorflow.google.cn/api_docs/python/tf/errors/OutOfRangeError), also cause  `sv.should_stop()`  to return  `True` 
...
depend on the <em>chief</em> for these services.

The only change you have to do to the single program code is to indicate
if the program is running as the <em>chief</em>.
...
In the <em>chief</em> task, the  `Supervisor`  works exactly as in the first example
...
appropriate to pass  `server.target`  (for some [ `tf.distribute.Server` ](https://tensorflow.google.cn/api_docs/python/tf/distribute/Server)
named `server).
...
Please switch to tf.train.MonitoredTrainingSession</aside>
...
the tensor returned from [ `tf.compat.v1.report_uninitialized_variables()` ](https://tensorflow.google.cn/api_docs/python/tf/compat/v1/report_uninitialized_variables)
...
[ `tf.compat.v1.Session` ](https://tensorflow.google.cn/api_docs/python/tf/compat/v1/Session) constructor for how this is interpreted.
...
execution is enabled, use the [ `tf.data` ](https://tensorflow.google.cn/api_docs/python/tf/data) API.
...
the Supervisor's Coordinator to join these threads with:
  sv.coord.Join(<list of="" threads="">)</list>
