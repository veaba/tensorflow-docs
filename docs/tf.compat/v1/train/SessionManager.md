...
- `init_feed_dict`: Optional dictionary that maps `Tensor` objects to feed values. This feed dictionary is passed to the session `run`() call when `run`ning the init op.
