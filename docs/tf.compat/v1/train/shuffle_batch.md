...
Queue-based input pipelines have been replaced by [ `tf.data` ](https://tensorflow.google.cn/api_docs/python/tf/data). Use  `tf.data.Dataset.shuffle(min_after_dequeue).batch(batch_size)` .</aside>
...
<em>N.B.:</em> You must ensure that either (i) the  `shapes`  argument is
