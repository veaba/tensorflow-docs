...
- `export_scope`: Optional `string`. Name scope under which to extract the sub`graph`. The scope name will be striped from the node definitions for easy import later into new name scopes. If `None`, the whole `graph` is exported. `graph_def` and `export_scope` cannot both be specified.
- `clear_devices`: Whether or not to clear the device field for an `Operation` or `Tensor` during export.
- `clear_extraneous_savers`: Remove any Saver-related information from the `graph` (both Save/Restore ops and `SaverDef`s) that are not associated with the provided `SaverDef`.
...
- `save_debug_info`: If `True`, save the `Graph`DebugInfo to a separate file, which in the same directory of `filename` and with `_debug` added before the file extend.
#### Returns:
