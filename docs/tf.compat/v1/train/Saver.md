...
[Variables](https://tensorflow.org/guide/variables)See  for an overview of variables, saving and restoring.
...
- `max_to_keep` indicates the maximum number of recent `checkpoint` files to keep. As new files are created, older files are deleted. If None or 0, no `checkpoint`s are deleted from the filesystem but only the last one is kept in the `checkpoint` file. Defaults to 5 (that is, the 5 most recent `checkpoint` files are kept.)
...
- `builder`: Optional `Saver`Builder to use if a `saver_def` was not provided. Defaults to `BulkSaverBuilder`().
...
[tf.train.Checkpoint](https://www.tensorflow.org/api_docs/python/tf/train/Checkpoint)Although Saver works in some cases when executing eagerly, it is fragile. Please switch to  or tf.keras.Model.save_weights, which perform a more robust object-based saving. These APIs will load checkpoints written by Saver.
