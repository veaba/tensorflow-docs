...
[tf.train.Checkpoint](https://tensorflow.google.cn/api_docs/python/tf/train/Checkpoint)Although Saver works in some cases when executing eagerly, it is fragile. Please switch to  or tf.keras.Model.save_weights, which perform a more robust object-based saving. These APIs will load checkpoints written by Saver.
