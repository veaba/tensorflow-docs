...
- `partition_strategy`: A string specifying the partitioning strategy, relevant if len(params) > 1. Currently `"div"` and `"mod"` are supported. Default is `"mod"`. See `` for more details.
