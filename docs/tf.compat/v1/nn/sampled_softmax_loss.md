...
GitHub
](https://github.com/tensorflow/tensorflow/blob/r2.0/tensorflow/python/ops/nn_impl.py#L2127-L2225)  
---  
...
#### Args:
  ...
  * **`labels`** : A `Tensor` of type `int64` and shape `[batch_size, num_true]`. The target classes. Note that this format differs from the `labels` argument of [`nn.softmax_cross_entropy_with_logits`](/api_docs/python/tf/nn/softmax_cross_entropy_with_logits).
