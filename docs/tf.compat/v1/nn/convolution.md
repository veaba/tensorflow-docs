...
[N](https://tensorflow.google.cn/api_docs/python/tf/compat/v1/nn/defaulting%20to%20%5B1%5D*N)an optional dilation_rate tensor of shape  specifying the filter upsampling/input downsampling rate, and an optional list of  strides (defaulting [1]*), this computes for each -D spatial output position (x[0], ..., x[-1]):
