...
- **`loop_fn`** : A callable that takes inputs <code translate="no" dir="ltr">(time, cell_output, cell_state,
loop_state)</code> and returns the tuple <code translate="no" dir="ltr">(finished, next_input,
next_cell_state, emit_output, next_loop_state)</code>. Here  `time`  is an int32
...
must be a  `Tensor`  of appropriate type and shape <code translate="no" dir="ltr">[batch_size,
cell.state_size]</code>. If  `cell.state_size`  is a  `TensorShape` , this must be
a  `Tensor`  of appropriate type and shape <code translate="no" dir="ltr">[batch_size] +
cell.state_size</code>. If  `cell.state_size`  is a (possibly nested) tuple of
...
nested) tuple structure of tensors, e.g., <code translate="no" dir="ltr">(tf.zeros(shape_0,
