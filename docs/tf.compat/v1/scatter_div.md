...
Duplicate entries are handled correctly: if multiple  `indices`  referencethe same location, their contributions divide.
...
- **`updates`** : A  `Tensor` . Must have the same type as  `ref` . A tensor of valuesthat  `ref`  is divided by.
