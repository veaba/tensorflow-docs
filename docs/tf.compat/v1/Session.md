...
[tf.Graph()](https://www.tensorflow.org/api_docs/python/tf/Graph)If no graph argument is specified when constructing the session, the default graph will be launched in the session. If you are using more than one graph (created with ) in the same process, you will have to use different sessions for each graph, but each graph can be used in multiple sessions. In this case, it is often clearer to pass the graph to be launched explicitly to the session constructor.
...
- `config`: (Optional.) A `ConfigProto` protocol buffer with `config`uration options for the session.
## Properties
### graph
...
[tf.Operation.run](https://www.tensorflow.org/api_docs/python/tf/Operation#run)Use with the with keyword to specify that calls to  or tf.Tensor.eval should be executed in this session.
...
with sess.as_default():
  ...
  print(c.eval())
```
[tf.compat.v1.get_default_session](https://www.tensorflow.org/api_docs/python/tf/compat/v1/get_default_session)To get the current default session, use .
...
[tf.Tensor](https://www.tensorflow.org/api_docs/python/tf/Tensor)The returned callable will take len(feed_list) arguments whose types must be compatible feed values for the respective elements of feed_list. For example, if element i of feed_list is a , the ith argument to the returned callable must be a numpy ndarray (or something convertible to an ndarray) with matching element type and shape. See tf.Session.run for details of the allowable feed key and value types.

[tf.Tensor](https://www.tensorflow.org/api_docs/python/tf/Tensor)The returned callable will have the same return type as tf.Session.run(fetches, ...). For example, if fetches is a , the callable will return a numpy ndarray; if fetches is a tf.Operation, it will return None.
...
- `config`: (Optional.) Protocol buffer with `config`uration options.
#### Raises:
