...
[tf.Tensor.eval](https://www.tensorflow.org/api_docs/python/tf/Tensor#eval)The only difference with a regular Session is that an InteractiveSession installs itself as the default session on construction. The methods  and tf.Operation.run will use that session to run ops.
...
- `config`: (Optional) `ConfigProto` proto used to `config`ure the session.
## Properties
### graph
