...
<devsite-code no-copy=""><pre translate="no" dir="ltr" is-upgraded=""><code translate="no" dir="ltr">pdf(x; alpha, beta) = x**(alpha - 1) (1 - x)**(beta - 1) / Z
...
<devsite-code no-copy=""><pre translate="no" dir="ltr" is-upgraded=""><code translate="no" dir="ltr">concentration1 = alpha = mean * total_concentration
...
<aside class="warning">**Warning:**  The samples can be zero due to finite precision.
...
<aside class="note">**Note:**   `x`  must have dtype  `self.dtype`  and be in
 `[0, 1].`  It must have a shape compatible with  `self.batch_shape()` .</aside>
...
<aside class="note">**Note:**  The mode is undefined when  `concentration1 <= 1`  or
 `concentration0 <= 1` . If  `self.allow_nan_stats`  is  `True` ,  `NaN` 
is used for undefined modes. If  `self.allow_nan_stats`  is  `False`  an
exception is raised when one or more modes are undefined.</aside>
