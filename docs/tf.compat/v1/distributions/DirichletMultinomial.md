...
<devsite-code no-copy=""><pre translate="no" dir="ltr" is-upgraded=""><code translate="no" dir="ltr">pmf(n; alpha, N) = Beta(alpha + n) / (prod_j n_j!) / Z
Z = Beta(alpha) / N!
...
<aside class="note">**Note:**  This condition is validated only when  `self.validate_args = True` .</aside>
...
<devsite-code no-copy=""><pre translate="no" dir="ltr" is-upgraded=""><code translate="no" dir="ltr">Var(X_j) = n * alpha_j / alpha_0 * (1 - alpha_j / alpha_0) *
(n + alpha_0) / (1 + alpha_0)
</code></pre></devsite-code>
...
<devsite-code no-copy=""><pre translate="no" dir="ltr" is-upgraded=""><code translate="no" dir="ltr">Cov(X_i, X_j) = -n * alpha_i * alpha_j / alpha_0 ** 2 *
(n + alpha_0) / (1 + alpha_0)
</code></pre></devsite-code>
...
<aside class="note">**Note:**   `value`  must be a non-negative tensor with dtype  `self.dtype` , have no
fractional components, and such that
 `tf.reduce_sum(value, -1) = self.total_count` . Its shape must be broadcastable
with  `self.concentration`  and  `self.total_count` .</aside>
