...
- `seed`: An optional `int`. Defaults to `0`. If either `seed` or `seed`2 are set to non-zero, the random number generator is `seed`ed by the given `seed`. Otherwise, it is `seed`ed by a random `seed`.
