...
tensor = tf.constant(-1.0, shape=[2, 3]) => [[-1. -1. -1.]
                                             [-1. -1. -1.]]
```
[tf.constant](https://www.tensorflow.org/api_docs/python/tf/constant) differs from tf.fill in a few ways:
...
- `verify_shape`: Boolean that enables verification of a `shape` of `value`s.
#### Returns:
