...
- **`data_source_name`** : A 0-D [ `tf.string` ](https://tensorflow.google.cn/api_docs/python/tf#string) tensor containing a connection stringto connect to the database.
- **`query`** : A 0-D [ `tf.string` ](https://tensorflow.google.cn/api_docs/python/tf#string) tensor containing the SQL query to execute.
- **`output_types`** : A tuple of [ `tf.DType` ](https://tensorflow.google.cn/api_docs/python/tf/dtypes/DType) objects representing the types of thecolumns returned by  `query` .
...
NOTE: This is an escape hatch for existing uses of  `filter`  that do not workwith V2 functions. New uses are strongly discouraged and existing usesshould migrate to  `filter`  as this method will be removed in V2.
...
NOTE: This is an escape hatch for existing uses of  `map`  that do not workwith V2 functions. New uses are strongly discouraged and existing usesshould migrate to  `map`  as this method will be removed in V2.
