...
[Strategy](https://www.tensorflow.org/api_docs/python/tf/compat/v1/distribute/Strategy)Inherits From: 
...
[tf.distribute.get_replica_context()](https://www.tensorflow.org/api_docs/python/tf/distribute/get_replica_context)fn may call  to access members such as replica_id_in_sync_group.

[tf.distribute.Strategy](https://www.tensorflow.org/api_docs/python/tf/distribute/Strategy)IMPORTANT: Depending on the  implementation being used, and whether eager execution is enabled, fn may be called one or more times (once for each replica).
...
DEPRECATED: This method is not available in TF 2.x.
[tf.distribute.InputContext](https://www.tensorflow.org/api_docs/python/tf/distribute/InputContext)The input_fn should take an  object where information about batching and input sharding can be accessed:
...
```
[tf.data.Dataset](https://www.tensorflow.org/api_docs/python/tf/data/Dataset)The  returned by input_fn should have a per-replica batch size, which may be computed using input_context.get_per_replica_batch_size.
